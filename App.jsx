...
const FAQ_DB = [
  ...
  { id: "ua1", category: "Using This App", q: "How does the photo scan estimate calories?", keywords: ["how photo scan works", "how does scanning work"], a: "When you snap or upload a photo, it's sent to the AI once to identify the food and estimate the portion's calories and macros. You can then edit every number before it's added to your log — the AI's guess is a starting point, not the final word." },
  { id: "ua2", category: "Using This App", q: "How was my calorie/macro plan calculated?", keywords: ["how plan calculated", "how was my plan made"], a: "Your plan came from the one-time intake conversation — the AI used your body stats plus what you told it about your goal, activity, and lifestyle to set targets. You can also edit the numbers directly anytime from the Profile tab." },
  ...
  { id: "ua4", category: "Using This App", q: "Why does this coach say it can't answer something?", keywords: ["why no answer", "coach cant answer"], a: "This coach answers from a fixed list of common questions with zero AI cost, so very specific or oddly-phrased questions may not match anything. Use the \"Ask real AI\" option on an unmatched question if you want a fuller answer — that one does use your Claude usage." },
];
...
async function callClaude({ system, messages }, attempt = 0) {
  const body = { system, messages, max_tokens: 700 };
  let res;
  try {
  ...
  try {
    ...
    throw new Error(`Bad response from AI (status ${res.status}).`);
  }
  if (data.error) {
    const type = data.error.type || "";
    const msg = data.error.message || "";
    const transient = res.status === 429 || res.status === 529 || type === "overloaded_error";
    const isLimit = type === "rate_limit_error" || res.status === 429;
    if (transient && !isLimit && attempt < 2) {
      await new Promise((r) => setTimeout(r, 1500 * (attempt + 1)));
      return callClaude({ system, messages }, attempt + 1);
    }
    if (isLimit) {
      const err = new Error("Your Anthropic API key has hit its rate limit or spending cap. Check usage/limits at console.anthropic.com, or wait a bit and try again.");
      err.isLimit = true;
      throw err;
    }
    ...
    throw new Error(msg || "AI request was rejected.");
  }
  ...
  return (
    <div className="nt-modal-backdrop" onClick={onClose}>
      <div className="nt-limit-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <h2>You've hit your Claude usage limit</h2>
        <p>Every AI feature here — photo scanning, the coach's "Ask real AI," and plan setup — shares the same usage pool as your regular Claude conversations. That pool has run dry for now.</p>
        <p className="nt-limit-sub">It refills on a timer, usually within a few hours. Free plan quotas are smaller than Pro/Max. Everything else in the app — logging food manually, water, exercise, weight, and the free coach Q&A — still works normally.</p>
    ...
    <div className="nt-form-wrap">
      ...
      <div className="nt-warning-list">
        ...
        <div className="nt-warning-item">
          ...
          <p>Photo scanning, building your plan, and the coach's "Ask real AI" all call Anthropic's API through this site's own key — separate from any personal Claude subscription you may have elsewhere.</p>
        </div>
        <div className="nt-warning-item">
...
function WeightTab({ basics, plan, weightLog, onAddWeight, onDeleteWeight, checking, dailyTotals, onCheckAdjustment, measurements, onAddMeasurement, onDeleteMeasurement }) {
  ...
  return (
    <div className="nt-page">
      ...
      <p className="nt-note">This uses one AI check from your Claude usage — nothing runs automatically in the background anymore.</p>
...
function AssistantTab({ chat, setChat, context, onUpdatePlan, onLimitReached }) {
  ...
  const send = () => {
    ...
    if (results.length > 0) {
      ...
      pushPair(text, { id: uid(), role: "assistant", content: "I don't have a canned answer for that — try browsing topics below, rephrase it, or ask the real AI.", canAskAI: true, originalQuery: text });
    }
  };
      ...
      <div className="nt-chat-scroll">
        ...
        {chat.map((m, i) => (
          <div key={m.id || i} className={`nt-bubble ${m.role}`}>
            {m.content}
            {m.aiPowered && <span className="nt-ai-tag">AI</span>}
            {m.canAskAI && <button className="nt-asktextbtn" disabled={asking} onClick={() => askRealAI(m.originalQuery)}>Ask real AI instead →</button>}
          </div>
        ))}
...
function ScanFood({ onAddMeals, onLimitReached }) {
  ...
  const analyze = async () => {
    ...
    try {
      const text = await callClaude({
        system: `You identify food items in a photo and estimate their nutrition. Respond with ONLY valid JSON, no markdown fences, no preamble. Format: {"items":[{"name":string,"portion":string,"cal":number,"p":number,"c":number,"f":number}]}. If Indian food, use accurate typical Indian recipe estimates.`,
        messages: [{ role: "user", content: [
          { type: "image", source: { type: "base64", media_type: img.type, data: img.data } },
          { type: "text", text: "Identify the food(s) in this photo and estimate calories and macros for the portion shown." },
        ] }],
      });
      const clean = text.replace(/```json|```/g, "").trim();
      ...
      if (e.isLimit) { onLimitReached(); } else { setError(e.message || "Couldn't read that photo — try again or add manually."); }
  ...
  return (
    <div className="nt-modal-body">
      ...
      {img && <button className="nt-btn primary full" onClick={analyze} disabled={analyzing}>{analyzing ? <><Loader2 className="nt-spin" size={16} /> Analyzing…</> : "Analyze photo"}</button>}
    </div>
  );
}
